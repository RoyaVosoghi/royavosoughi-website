    ...
    <>
      <Section tone="light" className="pt-14 md:pt-20">
        <div className="max-w-3xl">
          ...
          <p className="mt-8 text-xl text-navy/80">{t("lead")}</p>
        </div>

        <div className="mt-14 grid gap-12 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)] lg:gap-16">
          <div className="space-y-5 text-lg text-navy/80">
            <p>{t("p1")}</p>
            <p>{t("p2")}</p>
            <p>{t("p3")}</p>

            {/* Placeholder until the CV and photos arrive — see README */}
            <div className="rounded-2xl border-2 border-dashed border-navy/20 bg-mist/40 p-6 text-base text-navy/70">
              {t("cvPlaceholder")}
            </div>
          </div>

          <aside>
            <h2 className="label-eyebrow text-coral-deep">{t("valuesTitle")}</h2>
            <dl className="mt-6 space-y-6">
              {values.map((key) => (
                <div key={key} className="border-s-2 border-coral/30 ps-5">
                  <dt className="font-semibold text-navy">
                    {t(`values.${key}.name`)}
                  </dt>
                  <dd className="mt-1.5 text-navy/75">
                    {t(`values.${key}.body`)}
                  </dd>
                </div>
              ))}
      ...
      <Section tone="forest">
        <div className="text-center">
          <h2 className="text-section text-canvas">{t("ctaTitle")}</h2>
