...
export function BudgetConfigForm({
  ...
  return (
    <section className="rounded-3xl border-2 border-soft/10 bg-indigo p-6">
      ...
      <p className="mt-3 font-display text-2xl font-bold text-soft">
        ${spentUsd.toFixed(2)}
        <span className="text-sm font-normal text-soft/50">
          {" "}
          {t("spentThisMonth")}
      ...
      {overThreshold ? (
        <p className="mt-1 text-sm font-semibold text-amber-soft">{t("overThreshold", { threshold: alertThresholdPct })}</p>
      ) : null}
      {crmAiUsd > 0 ? <p className="mt-1 text-xs text-soft/50">{t("crmAiUsdNote", { amount: crmAiUsd.toFixed(2) })}</p> : null}
