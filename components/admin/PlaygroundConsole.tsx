      ...
      <div className="rounded-3xl border-2 border-navy/10 bg-canvas p-6">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <label className="text-xs font-medium text-navy/70">
            {t("localeLabel")}
            <select value={locale} onChange={(e) => setLocale(e.target.value as "en" | "fa")} className="mt-1 w-full rounded-xl border-2 border-navy/15 bg-canvas px-3 py-2 text-sm text-navy focus:border-coral focus:outline-none">
              <option value="en">{t("localeEnglish")}</option>
              <option value="fa">{t("localeFa")}</option>
            </select>
          </label>
          <label className="flex items-end gap-2 text-xs font-medium text-navy/70">
          ...
          <label className="text-xs font-medium text-navy/70">
            ...
            <select value={modelA} onChange={(e) => setModelA(e.target.value)} className="mt-1 w-full rounded-xl border-2 border-navy/15 bg-canvas px-3 py-2 text-sm text-navy focus:border-coral focus:outline-none">
          ...
          {compare ? (
            <label className="text-xs font-medium text-navy/70">
              {t("modelBLabel")}
              <select value={modelB} onChange={(e) => setModelB(e.target.value)} className="mt-1 w-full rounded-xl border-2 border-navy/15 bg-canvas px-3 py-2 text-sm text-navy focus:border-coral focus:outline-none">
                {groupedModelCatalog().map((g) => (
                  <optgroup key={g.group} label={g.label}>
                    {g.models.map((m) => (
                      <option key={m.slug} value={m.slug}>{m.label}</option>
          ...
          <label className="text-xs font-medium text-navy/70">
            {t("temperatureLabel")}
            <input type="number" min={0} max={2} step={0.1} value={temperature} onChange={(e) => setTemperature(Number(e.target.value))} className="mt-1 w-full rounded-xl border-2 border-navy/15 bg-canvas px-3 py-2 text-sm text-navy focus:border-coral focus:outline-none" />
          </label>
          <label className="text-xs font-medium text-navy/70">
            {t("topPLabel")}
            <input type="number" min={0} max={1} step={0.05} value={topP} onChange={(e) => setTopP(Number(e.target.value))} className="mt-1 w-full rounded-xl border-2 border-navy/15 bg-canvas px-3 py-2 text-sm text-navy focus:border-coral focus:outline-none" />
          </label>
          <label className="text-xs font-medium text-navy/70">
            {t("maxTokensLabel")}
            <input type="number" min={64} max={8192} value={maxTokens} onChange={(e) => setMaxTokens(Number(e.target.value))} className="mt-1 w-full rounded-xl border-2 border-navy/15 bg-canvas px-3 py-2 text-sm text-navy focus:border-coral focus:outline-none" />
          </label>
          <label className="text-xs font-medium text-navy/70 sm:col-span-2">
        ...
        {status === "error" ? <p className="mt-2 text-sm font-medium text-amber-deep">{t("runError")}</p> : null}
      </div>

      {results ? (
        <div className={`grid grid-cols-1 gap-6 ${results.length > 1 ? "lg:grid-cols-2" : ""}`}>
          {results.map((r, i) => (
            <div key={i} className="rounded-3xl border-2 border-navy/10 bg-canvas p-6">
              <p className="font-display text-sm font-bold text-navy">{r.model}</p>
              <p className="mt-1 text-xs text-navy/50">
                {t("tokensLine", { tokensIn: r.tokensIn ?? "?", tokensOut: r.tokensOut ?? "?" })}
                {r.toolCalls.length ? t("toolCallsSuffix", { calls: r.toolCalls.join(", ") }) : ""}
              </p>
              <p className="mt-3 whitespace-pre-wrap text-navy/85">{r.reply || t("emptyReply")}</p>

              {r.retrievedChunks.length > 0 ? (
                <details className="mt-4 text-xs text-navy/60">
                  ...
                  <ul className="mt-2 flex flex-col gap-2">
                    {r.retrievedChunks.map((c) => (
                      <li key={c.id} className="rounded-lg bg-mist/30 p-2">
                ...
                <p className="mt-4 text-xs text-navy/40">{t("noChunksRetrieved")}</p>
              )}
            </div>
          ))}
        </div>
