...
export function ManualReplyBox({ conversationId, paused }: { conversationId: string; paused: boolean }) {
  ...
  return (
    <form onSubmit={onSubmit} className="mt-4 rounded-2xl border-2 border-soft/10 bg-violet/8 p-4">
      {!paused ? (
        <p className="mb-2 text-xs font-medium text-amber-soft">{t("activeWarning")}</p>
      ) : null}
      <div className="flex gap-3">
        <textarea
          ...
          className="flex-1 rounded-xl border-2 border-soft/15 bg-indigo px-3 py-2 text-sm text-soft focus:border-cyan focus:outline-none"
        />
        <button
          type="submit"
      ...
      {status === "error" ? <p className="mt-2 text-sm font-medium text-amber-soft">{t("error")}</p> : null}
    </form>
  );
}
