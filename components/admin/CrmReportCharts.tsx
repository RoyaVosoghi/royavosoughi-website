    ...
    <div className="h-64 w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 8, right: 8, bottom: 8, left: -16 }}>
          ...
          <XAxis dataKey="status" tick={{ fontSize: 12, fill: "#14213d" }} tickLine={false} axisLine={{ stroke: "#14213d1f" }} />
          <YAxis tick={{ fontSize: 12, fill: "#14213d" }} tickLine={false} axisLine={false} allowDecimals={false} />
          <Tooltip contentStyle={{ borderRadius: 12, border: "1px solid #14213d22", background: "#ffffff", color: "#14213d", fontSize: 13 }} cursor={{ fill: "#e6e6e6" }} />
      ...
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} layout="vertical" margin={{ top: 8, right: 24, bottom: 8, left: 8 }}>
          ...
          <XAxis
            ...
            tickLine={false}
            axisLine={{ stroke: "#14213d1f" }}
            tickFormatter={(v) => `$${v}`}
          />
          <YAxis type="category" dataKey="stage" width={120} tick={{ fontSize: 11, fill: "#14213d" }} tickLine={false} axisLine={false} />
          <Tooltip
    ...
    <div className="h-64 w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={months} margin={{ top: 8, right: 8, bottom: 8, left: -16 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#14213d1f" vertical={false} />
          <XAxis dataKey="label" tick={{ fontSize: 12, fill: "#14213d" }} tickLine={false} axisLine={{ stroke: "#14213d1f" }} />
          <YAxis tick={{ fontSize: 12, fill: "#14213d" }} tickLine={false} axisLine={false} tickFormatter={(v) => `$${v}`} />
          <Tooltip
